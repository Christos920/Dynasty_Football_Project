...
if pca is not None:
    ...
    **PCA Axes:**  
    ...
    st.markdown("**PCA not available for this group (not enough data).**")

sel1 = df_pos[df_pos['player_name'] == player1]
sel2 = df_pos[df_pos['player_name'] == player2]

# ---- PCA CHART (show both players) ----
if pca is not None and not df_pos['PC1'].isnull().all():
...
if len(sel1) and len(sel2):
    ...
    for i, (sel, color, label) in enumerate(zip([sel1, sel2], ['red', 'blue'], [player1, player2])):
        with cols[i]:
            ...
            metrics = [m for m in non_nan_cols if sel.iloc[0][m] == sel.iloc[0][m]]
